...
RUN apt install git build-essential sbcl
# Build SWCL
...
ENTRYPOINT ["/root/swcl/run-sbcl.sh", "--eval", "(ql:quickload :regrind-ee)", "--eval", "(regrind-ee:start-regrind 4)", "--eval", "(regrind-ee:start-ui)"]